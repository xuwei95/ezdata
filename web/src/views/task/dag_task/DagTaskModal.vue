...
<script lang="ts" setup>
  ...
  import { formSchema } from './dag_task.data';
  import { getInfoById, saveOrUpdate } from '../task.api';
      ...
      try {
        ...
        if (res_data) {
          ...
          if (isCopy.value) {
            ...
            copyParams.value = res_data.params;
            console.log('copy35324523', formData, copyParams.value);
          }
        } else {
  ...
  async function handleSubmit(v) {
    try {
      ...
      if (isCopy.value) {
        // 复制任务，将任务配置带上后新增任务
        values.params = copyParams.value;
