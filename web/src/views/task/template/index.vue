  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item :label="$t('模板名称')" prop="name">
        <el-input v-model="queryParams.name" :placeholder="$t('请输入模板名称')" clearable style="width: 200px" @keyup.enter="handleQuery" />
      </el-form-item>
      <el-form-item :label="$t('模板编码')" prop="code">
        <el-input v-model="queryParams.code" :placeholder="$t('请输入模板编码')" clearable style="width: 200px" @keyup.enter="handleQuery" />
      </el-form-item>
      <el-form-item :label="$t('状态')" prop="status">
        <el-select v-model="queryParams.status" :placeholder="$t('模板状态')" clearable style="width: 160px">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['task:template:add']">{{ $t('新增') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['task:template:remove']">{{ $t('删除') }}</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="templateList" @selection-change="handleSelectionChange">
      ...
      <el-table-column :label="$t('执行器类型')" align="center" prop="runnerType">
        <template #default="scope">
          <el-tag :type="scope.row.runnerType === 2 ? 'danger' : 'success'">{{ scope.row.runnerType === 2 ? '动态代码' : '内置runner' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('内置')" align="center" prop="builtIn">
        <template #default="scope">
          <el-tag v-if="scope.row.builtIn === 1" type="info">{{ $t('内置') }}</el-tag>
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('操作')" align="center" width="160" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['task:template:edit']">{{ $t('修改') }}</el-button>
          <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['task:template:remove']">{{ $t('删除') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" fullscreen append-to-body class="template-fullscreen-dialog">
      <el-form ref="templateRef" :model="form" :rules="rules" label-width="150px">
        ...
        <el-form-item :label="$t('模板编码')" prop="code">
          <el-input v-model="form.code" :placeholder="$t('如 MyTask(唯一)')" :disabled="form.builtIn === 1" />
        </el-form-item>
        <el-form-item :label="$t('表单类型')" prop="type">
          <el-radio-group v-model="form.type">
            <el-radio :value="1">{{ $t('内置组件') }}</el-radio>
            <el-radio :value="2">{{ $t('动态配置') }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item :label="$t('执行器类型')" prop="runnerType">
          <el-radio-group v-model="form.runnerType" :disabled="form.builtIn === 1" @change="onRunnerTypeChange">
            <el-radio :value="1">{{ $t('内置runner') }}</el-radio>
            <el-radio :value="2">{{ $t('动态代码') }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item :label="$t('动态代码')" prop="runnerCode" v-if="form.runnerType === 2">
          <code-editor v-model="form.runnerCode" language="python" height="240px" :placeholder="$t('需定义 run(params, logger) 函数')" />
        </el-form-item>
        <!-- 动态配置(type=2)：可视化低代码设计参数表单；内置组件(type=1)：指定前端组件 -->
        <el-form-item v-if="form.type === 2" :label="$t('参数表单设计')" prop="params">
          <schema-designer v-model="form.params" />
        </el-form-item>
        <el-form-item v-else :label="$t('前端组件')" prop="component">
          <el-input v-model="form.component" :placeholder="$t('内置组件路径/名称，任务参数由该内置组件处理')" />
          <div class="form-tip">{{ $t('内置组件类型的模板由固定前端组件处理参数，任务表单不再动态配置') }}</div>
        </el-form-item>
