<template>
  <div class="metric-tab">
    <div class="mt-toolbar">
      <span class="hint">
        指标 = 绑定在本模型上的口径化度量(如「行业加权市盈率」)。AI 分析时可直接 query_metric 取权威一致的数,
        避免每次重写聚合。改动本模型会把相关指标标记为「待复核」。
      </span>
      <el-button type="primary" size="small" icon="Plus" @click="openAdd">新建指标</el-button>
    </div>

    <vxe-table :data="list" v-loading="loading" border height="360" size="small">
      <vxe-column field="code" title="代码" width="150" />
      <vxe-column field="name" title="名称" min-width="130" />
      <vxe-column title="度量" width="150">
        <template #default="{ row }">{{ measureText(row) }}</template>
      </vxe-column>
      <vxe-column title="维度" min-width="120">
      ...
      <vxe-column title="状态" width="90">
        <template #default="{ row }">
          ...
          <el-tag v-else-if="row.status === '0'" type="success" size="small">启用</el-tag>
          <el-tag v-else type="info" size="small">停用</el-tag>
        </template>
      </vxe-column>
      <vxe-column title="操作" width="180" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" size="small" @click="preview(row)">试跑</el-button>
          <el-button link type="primary" size="small" @click="openEdit(row)">编辑</el-button>
          <el-button link type="danger" size="small" @click="remove(row)">删除</el-button>
        </template>
      </vxe-column>
      <template #empty><el-empty description="本模型暂无指标,点「新建指标」创建" :image-size="70" /></template>
    </vxe-table>

    <!-- 新建 / 编辑 -->
    <el-dialog :title="form.metricId ? '编辑指标' : '新建指标'" v-model="dlg" width="640px" append-to-body>
      <el-form :model="form" label-width="92px">
        <el-form-item label="名称" required>
          <el-input v-model="form.name" placeholder="如:行业加权市盈率" />
        </el-form-item>
        <el-form-item label="代码" required>
          <el-input v-model="form.code" :disabled="!!form.metricId" placeholder="英文唯一,如 industry_pe_avg" />
        </el-form-item>
        <el-form-item label="度量" required>
          <el-select v-model="form.agg" style="width: 130px" placeholder="聚合">
          ...
          <el-select v-model="form.field" filterable allow-create style="width: 200px; margin-left: 8px"
            placeholder="度量字段" :disabled="form.agg === 'count'">
            <el-option v-for="f in fieldOpts" :key="f" :label="f" :value="f" />
          </el-select>
          <span class="tip">count 无需字段</span>
        </el-form-item>
        <el-form-item label="维度">
          <el-select v-model="form.dims" multiple filterable allow-create style="width: 100%"
            placeholder="分组维度字段(可多选,如 industry_name)">
            <el-option v-for="f in fieldOpts" :key="f" :label="f" :value="f" />
          </el-select>
        </el-form-item>
        <el-form-item label="时间字段">
          <el-select v-model="form.timeField" clearable filterable allow-create style="width: 260px"
            placeholder="用于时间范围过滤(如 date)">
            <el-option v-for="f in fieldOpts" :key="f" :label="f" :value="f" />
          </el-select>
          <span class="tip">口径的时间列,可留空</span>
        </el-form-item>
        <el-form-item label="单位"><el-input v-model="form.unit" style="width: 200px" placeholder="如 倍 / 元 / 亿" /></el-form-item>
        <el-form-item label="口径说明">
          <el-input v-model="form.caliber" type="textarea" :rows="2"
            placeholder="该指标的业务口径/计算说明,AI 与用户共用同一定义" />
        </el-form-item>
    ...
    <!-- 试跑结果 -->
    <el-dialog title="指标试跑" v-model="pvDlg" width="640px" append-to-body>
      <div v-if="pvData">
        <div class="pv-meta">
          <b>{{ pvData.code }}</b> · 单位 {{ pvData.unit || '—' }}
          <div v-if="pvData.caliber" class="pv-caliber">口径:{{ pvData.caliber }}</div>
        </div>
...
const AGGS = [
  { v: 'sum', t: '求和 sum' }, { v: 'avg', t: '平均 avg' }, { v: 'max', t: '最大 max' },
  { v: 'min', t: '最小 min' }, { v: 'count', t: '计数 count' }, { v: 'count_distinct', t: '去重计数' },
]
...
async function save() {
  if (!form.value.name || !form.value.code) { ElMessage.warning('名称、代码必填'); return }
  if (form.value.agg !== 'count' && !form.value.field) { ElMessage.warning('该聚合需选度量字段'); return }
  const payload = {
  ...
  saving.value = true
  try {
    if (form.value.metricId) { payload.metricId = form.value.metricId; await updateMetric(payload); ElMessage.success('已保存') }
    else { await addMetric(payload); ElMessage.success('已创建') }
    dlg.value = false
    load()
...
async function preview(row) {
  const loadingInst = ElMessage({ message: '试跑中…', type: 'info', duration: 0 })
  try {
