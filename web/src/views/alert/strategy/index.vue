  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="策略名称" prop="strategyName">
        <el-input v-model="queryParams.strategyName" placeholder="请输入策略名称" clearable style="width: 200px" @keyup.enter="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['alert:strategy:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['alert:strategy:remove']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="strategyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="业务" align="center" prop="biz" width="110" />
      <el-table-column label="告警等级" align="center" width="100">
      ...
      <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['alert:strategy:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['alert:strategy:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="720px" append-to-body>
      <el-form ref="strategyRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="策略名称" prop="strategyName">
          <el-input v-model="form.strategyName" placeholder="请输入策略名称" />
        </el-form-item>
        <el-form-item label="告警等级" prop="level">
          <el-radio-group v-model="form.level">
            <el-radio :value="0">信息</el-radio>
            <el-radio :value="1">警告</el-radio>
            <el-radio :value="2">错误</el-radio>
          </el-radio-group>
        </el-form-item>

        <el-divider content-position="left">转发渠道</el-divider>
        <div v-for="(ch, idx) in channels" :key="idx" class="channel-card">
          <div class="channel-head">
            <el-select v-model="ch.type" size="small" style="width: 160px" placeholder="渠道类型">
              <el-option v-for="c in channelTypes" :key="c.value" :label="c.label" :value="c.value" />
            </el-select>
            <el-button link type="danger" icon="Delete" @click="channels.splice(idx, 1)">移除</el-button>
          </div>
          <!-- webhook -->
          <template v-if="ch.type === 'webhook'">
            <el-input v-model="ch.webhook_url" placeholder="Webhook 地址 URL" size="small" class="mt6" />
            <el-row :gutter="8" class="mt6">
              <el-col :span="8">
                <el-select v-model="ch.webhook_method" size="small" placeholder="方法">
          ...
          <template v-if="ch.type === 'kafka'">
            ...
            <el-input v-model="ch.bootstrap_servers" placeholder="bootstrap_servers(逗号分隔)，如 kafka1:9092,kafka2:9092" size="small" class="mt6" />
          </template>
          <!-- 邮件 -->
          <template v-if="ch.type === 'email'">
            <el-row :gutter="8" class="mt6">
              <el-col :span="16"><el-input v-model="ch.host" placeholder="SMTP host" size="small" /></el-col>
              <el-col :span="8"><el-input v-model="ch.port" placeholder="端口(465)" size="small" /></el-col>
            </el-row>
            <el-row :gutter="8" class="mt6">
              <el-col :span="12"><el-input v-model="ch.user" placeholder="账号" size="small" /></el-col>
              <el-col :span="12"><el-input v-model="ch.password" placeholder="密码/授权码" size="small" type="password" /></el-col>
            </el-row>
            <el-input v-model="ch.to" placeholder="收件人(逗号分隔)" size="small" class="mt6" />
          </template>
        </div>
        <el-button size="small" icon="Plus" @click="channels.push({ type: 'webhook', webhook_method: 'POST' })">添加渠道</el-button>
