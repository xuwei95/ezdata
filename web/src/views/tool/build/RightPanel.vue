  ...
  <div class="right-board">
    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        <!-- 组件属性 -->
        <el-form v-show="currentTab === 'field' && showField" size="default" label-width="140px" label-position="top"
          style="">
          <el-form-item v-if="activeData.changeTag" :label="$t('组件类型')">
            <el-select v-model="activeData.tagIcon" :placeholder="$t('请选择组件类型')" :style="{ width: '100%' }" @change="tagChange">
          ...
          <el-form-item v-if="activeData.vModel !== undefined" :label="$t('字段名')">
            <el-input v-model="activeData.vModel" :placeholder="$t('请输入字段名（v-model）')" />
          </el-form-item>
          <el-form-item v-if="activeData.componentName !== undefined" :label="$t('组件名')">
            {{ activeData.componentName }}
          </el-form-item>
          <el-form-item v-if="activeData.label !== undefined" :label="$t('标题')">
            <el-input v-model="activeData.label" :placeholder="$t('请输入标题')" />
          </el-form-item>
          <el-form-item v-if="activeData.placeholder !== undefined" :label="$t('占位提示')">
            <el-input v-model="activeData.placeholder" :placeholder="$t('请输入占位提示')" />
          </el-form-item>
          ...
          <el-form-item v-if="activeData.span !== undefined" :label="$t('表单栅格')">
            <el-slider v-model="activeData.span" :max="24" :min="1" :marks="{ 12: '' }" @change="spanChange" />
          </el-form-item>
          <el-form-item v-if="activeData.layout === 'rowFormItem'" :label="$t('栅格间隔')">
            <el-input-number v-model="activeData.gutter" :min="0" :placeholder="$t('栅格间隔')" />
          </el-form-item>

          <el-form-item v-if="activeData.justify !== undefined" :label="$t('水平排列')">
          ...
          <el-form-item v-if="activeData.align !== undefined" :label="$t('垂直排列')">
          ...
          <el-form-item v-if="activeData.labelWidth !== undefined" :label="$t('标签宽度')">
            <el-input v-model.number="activeData.labelWidth" type="number" :placeholder="$t('请输入标签宽度')" />
          </el-form-item>
          ...
          <el-form-item v-if="activeData.vModel !== undefined" :label="$t('默认值')">
            <el-input :value="setDefaultValue(activeData.defaultValue)" :placeholder="$t('请输入默认值')"
              @input="onDefaultValueInput" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-checkbox-group'" :label="$t('至少应选')">
            <el-input-number :value="activeData.min" :min="0" :placeholder="$t('至少应选')"
              @input="$set(activeData, 'min', $event ? $event : undefined)" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-checkbox-group'" :label="$t('最多可选')">
            <el-input-number :value="activeData.max" :min="0" :placeholder="$t('最多可选')"
              @input="$set(activeData, 'max', $event ? $event : undefined)" />
          </el-form-item>
          <el-form-item v-if="activeData.prepend !== undefined" :label="$t('前缀')">
            <el-input v-model="activeData.prepend" :placeholder="$t('请输入前缀')" />
          </el-form-item>
          <el-form-item v-if="activeData.append !== undefined" :label="$t('后缀')">
            <el-input v-model="activeData.append" :placeholder="$t('请输入后缀')" />
          </el-form-item>
          <el-form-item v-if="activeData['prefix-icon'] !== undefined" :label="$t('前图标')">
            <el-input v-model="activeData['prefix-icon']" :placeholder="$t('请输入前图标名称')">
              <template #append>
                <el-button icon="Pointer" @click="openIconsDialog('prefix-icon')"> {{ $t('选择') }} </el-button>
              </template>
            </el-input>
          </el-form-item>
          <el-form-item v-if="activeData['suffix-icon'] !== undefined" :label="$t('后图标')">
            <el-input v-model="activeData['suffix-icon']" :placeholder="$t('请输入后图标名称')">
              <template #append>
                <el-button icon="Pointer" @click="openIconsDialog('suffix-icon')"> {{ $t('选择') }} </el-button>
              </template>
            </el-input>
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-cascader'" :label="$t('选项分隔符')">
          ...
          <el-form-item v-if="activeData.min !== undefined" :label="$t('最小值')">
            <el-input-number v-model="activeData.min" :placeholder="$t('最小值')" />
          </el-form-item>
          <el-form-item v-if="activeData.max !== undefined" :label="$t('最大值')">
            <el-input-number v-model="activeData.max" :placeholder="$t('最大值')" />
          </el-form-item>
          <el-form-item v-if="activeData.step !== undefined" :label="$t('步长')">
            <el-input-number v-model="activeData.step" :placeholder="$t('步数')" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-input-number'" :label="$t('精度')">
            <el-input-number v-model="activeData.precision" :min="0" :placeholder="$t('精度')" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-input-number'" :label="$t('按钮位置')">
          ...
          <el-form-item v-if="activeData['active-value'] !== undefined" :label="$t('开启值')">
            <el-input :value="setDefaultValue(activeData['active-value'])" :placeholder="$t('请输入开启值')"
              @input="onSwitchValueInput($event, 'active-value')" />
          </el-form-item>
          <el-form-item v-if="activeData['inactive-value'] !== undefined" :label="$t('关闭值')">
            <el-input :value="setDefaultValue(activeData['inactive-value'])" :placeholder="$t('请输入关闭值')"
              @input="onSwitchValueInput($event, 'inactive-value')" />
          </el-form-item>
          <el-form-item v-if="activeData.type !== undefined && 'el-date-picker' === activeData.tag" :label="$t('时间类型')">
            <el-select v-model="activeData.type" :placeholder="$t('请选择时间类型')" :style="{ width: '100%' }"
          ...
          <el-form-item v-if="activeData.accept !== undefined" :label="$t('文件类型')">
            <el-select v-model="activeData.accept" :placeholder="$t('请选择文件类型')" :style="{ width: '100%' }" clearable>
          ...
          <el-form-item v-if="activeData.fileSize !== undefined" :label="$t('文件大小')">
            <el-input v-model.number="activeData.fileSize" :placeholder="$t('请输入文件大小')">
          ...
          <el-form-item v-if="activeData.buttonText !== undefined" v-show="'picture-card' !== activeData['list-type']"
            :label="$t('按钮文字')">
            <el-input v-model="activeData.buttonText" :placeholder="$t('请输入按钮文字')" />
          </el-form-item>
          ...
          <el-form-item v-if="activeData['picker-options'] !== undefined" :label="$t('时间段')">
            <el-input v-model="activeData['picker-options'].selectableRange" :placeholder="$t('请输入时间段')" />
          </el-form-item>
          <el-form-item v-if="activeData.format !== undefined" :label="$t('时间格式')">
            <el-input :value="activeData.format" :placeholder="$t('请输入时间格式')" @input="setTimeValue($event)" />
          </el-form-item>
          <template v-if="['el-checkbox-group', 'el-radio-group', 'el-select'].indexOf(activeData.tag) > -1">
            ...
            <draggable :list="activeData.options" :animation="340" group="selectItem" handle=".option-drag"
              ...
              <template #item="{ element, index }">
                <div :key="index" class="select-item">
                  ...
                  <el-input v-model="element.label" :placeholder="$t('选项名')" size="small" />
                  <el-input :placeholder="$t('选项值')" size="small" :value="element.value"
            ...
            <div>
              <el-button icon="CirclePlus" style="margin-left: 8px; margin-top: 10px;" text bg type="primary"
                @click="addSelectItem"> {{ $t('添加选项') }} </el-button>
            </div>
            <el-divider />
          </template>
          ...
          <template v-if="['el-cascader'].indexOf(activeData.tag) > -1">
            ...
            <template v-if="activeData.dataType === 'dynamic'">
              <el-form-item :label="$t('标签键名')">
                <el-input v-model="activeData.labelKey" :placeholder="$t('请输入标签键名')" />
              </el-form-item>
              <el-form-item :label="$t('值键名')">
                <el-input v-model="activeData.valueKey" :placeholder="$t('请输入值键名')" />
              </el-form-item>
              <el-form-item :label="$t('子级键名')">
                <el-input v-model="activeData.childrenKey" :placeholder="$t('请输入子级键名')" />
              </el-form-item>
            </template>
            ...
            <div v-if="activeData.dataType === 'static'">
              <el-button icon="CirclePlus" style="margin-left: 0; margin-top: 10px;" type="primary" text bg
                @click="addTreeItem"> {{ $t('添加父级') }} </el-button>
            </div>
            <el-divider />
          </template>

          <el-form-item v-if="activeData.optionType !== undefined" :label="$t('选项样式')">
          ...
          <el-form-item v-if="activeData.border !== undefined && activeData.optionType === 'default'" :label="$t('是否带边框')">
            <el-switch v-model="activeData.border" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-color-picker'" :label="$t('颜色格式')">
            <el-select v-model="activeData['color-format']" :placeholder="$t('请选择颜色格式')" :style="{ width: '100%' }"
          ...
          <el-form-item v-if="activeData.size !== undefined &&
            (activeData.optionType === 'button' ||
              activeData.border ||
              activeData.tag === 'el-color-picker')" :label="$t('选项尺寸')">
            <el-radio-group v-model="activeData.size">
              <el-radio-button label="large"> {{ $t('较大') }} </el-radio-button>
              <el-radio-button label="default"> {{ $t('默认') }} </el-radio-button>
          ...
          <el-form-item v-if="activeData.tag === 'el-input-number'" :label="$t('严格步数')">
            <el-switch v-model="activeData['step-strictly']" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-cascader'" :label="$t('是否多选')">
            <el-switch v-model="activeData.props.props.multiple" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-cascader'" :label="$t('展示全路径')">
            <el-switch v-model="activeData['show-all-levels']" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-cascader'" :label="$t('可否筛选')">
            <el-switch v-model="activeData.filterable" />
          </el-form-item>
          <el-form-item v-if="activeData.clearable !== undefined" :label="$t('能否清空')">
            <el-switch v-model="activeData.clearable" />
          </el-form-item>
          <el-form-item v-if="activeData.showTip !== undefined" :label="$t('显示提示')">
            <el-switch v-model="activeData.showTip" />
          </el-form-item>
          <el-form-item v-if="activeData.multiple !== undefined" :label="$t('多选文件')">
          ...
          <el-form-item v-if="activeData.tag === 'el-select'" :label="$t('是否可搜索')">
            <el-switch v-model="activeData.filterable" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-select'" :label="$t('是否多选')">
            <el-switch v-model="activeData.multiple" @change="multipleChange" />
          </el-form-item>
          <el-form-item v-if="activeData.required !== undefined" :label="$t('是否必填')">
          ...
          <template v-if="activeData.layout === 'colFormItem'">
            ...
            <div>
              <el-button icon="CirclePlus" style="margin-left: 0; margin-top: 10px;" type="primary" text bg
                @click="addReg"> {{ $t('添加规则') }} </el-button>
            </div>
          </template>
        </el-form>
        <!-- 表单属性 -->
        <el-form v-show="currentTab === 'form'" label-width="140px" label-position="top">
          <el-form-item :label="$t('表单名')">
            <el-input v-model="formConf.formRef" :placeholder="$t('请输入表单名（ref）')" />
          </el-form-item>
          ...
          <el-form-item :label="$t('标签宽度')">
            <el-input-number v-model="formConf.labelWidth" :placeholder="$t('标签宽度')" />
          </el-form-item>
