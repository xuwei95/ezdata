...
class IntegrationsClient:
    ...
    # 全局 handler 连接缓存（单例模式）
    _handlers_cache: Optional[HandlersCache] = None

    def __init__(self, handlers_base_path: Optional[str] = None, enable_cache: bool = True, cache_ttl: int = 300):
        """
        初始化客户端

        Args:
            handlers_base_path: handlers 的基础路径，默认为 mindsdb.integrations.handlers
            enable_cache: 是否启用 handler 连接缓存，默认为 True
            cache_ttl: 缓存过期时间（秒），默认为 300 秒（5 分钟）
        """
        ...
        # 初始化全局连接缓存（单例）
        if self.enable_cache and IntegrationsClient._handlers_cache is None:
            IntegrationsClient._handlers_cache = HandlersCache(ttl=cache_ttl)
    ...
    def _get_handler_cache_key(self, handler_type: str, connection_data: Dict[str, Any]) -> str:
        """
        生成 handler 的缓存键
    ...
    def get_or_create_handler(
        ...
        **kwargs
        ...
        """
        从缓存获取或创建一个新的 handler 实例

        如果启用了缓存且缓存中存在对应的 handler，则直接返回缓存的 handler。
        否则创建新的 handler 实例。
        ...
        Returns:
            BaseHandler 实例
        """
        # 如果没有启用缓存，直接创建新的 handler
        if not self.enable_cache or IntegrationsClient._handlers_cache is None:
            return self.create_handler(handler_type, connection_data, name, **kwargs)

        # 生成缓存键
        cache_key = self._get_handler_cache_key(handler_type, connection_data)
        ...
        # 将新创建的 handler 放入缓存
        # 需要设置 handler.name 属性为缓存键，以便缓存系统使用
        original_name = handler.name
        handler.name = cache_key
        IntegrationsClient._handlers_cache.set(handler)
        handler.name = original_name  # 恢复原始名称

        return handler

    def release_handler(self, handler_type: str, connection_data: Dict[str, Any]) -> None:
        """
        将 handler 释放回缓存（实际上不需要手动释放，因为缓存会自动管理）

        这个方法主要用于显式地从缓存中移除 handler 连接。
        ...
        logger.debug(f"从缓存中移除 handler: {cache_key}")
...
class StandaloneHandler:
    ...
    def get_columns(self, table_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            if hasattr(response, 'data_frame'):
                return response.data_frame
            else:
                return pd.DataFrame()
        except Exception as e:
