...
def retrieve(
    ...
    *,
    ...
    try:
        ...
        qa = (
            db.execute(
                select(RagChunk).where(
                    ...
                    RagChunk.question_hash == md5(query),
                    RagChunk.dataset_id.in_(dataset_ids),
                )
            )
            .scalars()
            .first()
        ...
        ds_map = {d.id: d for d in db.execute(select(RagDataset).where(RagDataset.id.in_(dataset_ids))).scalars().all()}
        ...
        def _qvec(ds: RagDataset) -> Any:
            key = (
                ds.embedding_provider or RagConfig.embedding_type,
            ...
            if retrieval_type == 'keyword':
                ...
                if retrieval_type == 'vector':
                    ...
                    hits = store.hybrid_search(
                        qvec, query, k * 2, filters=tenant_filter, score_threshold=score_threshold
                    )
        ...
        qa_map: dict[str, Any] = {}
        if qa_ids:
            qa_map = {c.id: c for c in db.execute(select(RagChunk).where(RagChunk.id.in_(qa_ids))).scalars().all()}
        for h in top:
            rec = {
