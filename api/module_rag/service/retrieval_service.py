...
class RetrievalService:
    ...
    async def search(cls, db: AsyncSession, req: RetrievalReq) -> dict:
        ...
        doc_ids = {r.get('document_id') for r in records if r.get('document_id') and r['document_id'] not in _PSEUDO_DOC}
        name_map: dict[str, str] = {}
        if doc_ids:
            rows = (await db.execute(
                select(RagDocument.id, RagDocument.name).where(RagDocument.id.in_(doc_ids)))).all()
