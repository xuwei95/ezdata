...
class LoginService:
    ...
    async def get_current_user(
        ...
        if token == redis_token:
            ...
            # 多租户:平台超管绕过可见全部;其余用户按 JWT 激活租户(经成员表校验)设上下文
    ...
    async def _resolve_active_tenant(
        ...
        with tenant_bypass():
            ...
            default_tid = await UserTenantDao.get_default_tenant(query_db, user_id)
        if default_tid is not None:
            return default_tid
        return cls._resolve_tenant_id(current_user)
