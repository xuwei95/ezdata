...
connection_args: 'OrderedDict[str, dict]' = OrderedDict(
    base_url={
        'type': ARG_TYPE.URL,
        'label': '接口根地址',
        'description': '如 https://api.example.com(取数路径相对它拼接)',
        'required': True,
    },
    auth_type={'type': ARG_TYPE.STR, 'label': '鉴权方式', 'description': 'none / bearer / api_key / basic(默认 none)'},
    token={
        'type': ARG_TYPE.PWD,
        'label': 'Token / API Key',
        'description': 'bearer 或 api_key 方式的密钥值',
        'secret': True,
    },
    api_key_header={
        'type': ARG_TYPE.STR,
