...
dag_controller = APIRouterPro(
    prefix='/task/dag', order_num=12, tags=['任务调度-DAG工作流'], dependencies=[PreAuthDependency()]
)
...
@dag_controller.get(
    '/{dag_id}/detail', summary='DAG 本体详情', dependencies=[UserInterfaceAuthDependency('task:dag:list')]
)
...
@dag_controller.put(
    '/{dag_id}/settings', summary='更新 DAG 本体设置', dependencies=[UserInterfaceAuthDependency('task:dag:edit')]
)
...
# ---------------- 发布 / 版本 / 回滚 ----------------
@dag_controller.post(
    '/{dag_id}/publish', summary='发布', dependencies=[UserInterfaceAuthDependency('task:dag:publish')]
)
...
@dag_controller.get(
    '/{dag_id}/versions', summary='版本列表', dependencies=[UserInterfaceAuthDependency('task:dag:list')]
)
...
@dag_controller.get(
    '/version/{ver_id}', summary='读某版本图', dependencies=[UserInterfaceAuthDependency('task:dag:list')]
)
async def dag_version_graph(
    ver_id: Annotated[str, Path()], db: Annotated[AsyncSession, DBSessionDependency()]
) -> Response:
    return ResponseUtil.success(data=await DagService.get_version_graph(db, ver_id))


@dag_controller.post(
    '/{dag_id}/rollback/{ver_id}', summary='回滚版本', dependencies=[UserInterfaceAuthDependency('task:dag:publish')]
)
async def dag_rollback(
    dag_id: Annotated[str, Path()],
    ver_id: Annotated[str, Path()],
    db: Annotated[AsyncSession, DBSessionDependency()],
) -> Response:
...
@dag_controller.post(
    '/{dag_id}/debug', summary='试运行(跑草稿)', dependencies=[UserInterfaceAuthDependency('task:dag:run')]
)
...
@dag_controller.get(
    '/run/{run_id}', summary='运行节点状态', dependencies=[UserInterfaceAuthDependency('task:dag:list')]
)
async def dag_run_status(
    run_id: Annotated[str, Path()], db: Annotated[AsyncSession, DBSessionDependency()]
) -> Response:
    return ResponseUtil.success(data=await DagService.run_nodes_status(db, run_id))


@dag_controller.get(
    '/{dag_id}/runs', summary='DAG 运行历史', dependencies=[UserInterfaceAuthDependency('task:dag:list')]
)
