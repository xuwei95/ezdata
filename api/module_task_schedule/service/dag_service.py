...
class DagService:
    ...
    @classmethod
    async def get_list(
        cls, db: AsyncSession, name: str | None, page_num: int, page_size: int, is_page: bool = True
    ) -> Any:
        query = (
            select(Task)
            .where(Task.task_type == 2, Task.name.like(f'%{name}%') if name else True)
        ...
        try:
            ...
            db.add(
                Task(
                    ...
                    status=1,
                    trigger_type=1,
                    run_queue=req.run_queue or 'default',
                    remark=req.remark,
                    create_by=operator,
            ...
            await DagGraphDao.add(
                db,
                {
                    ...
                    'version': 'draft',
                    'status': 'draft',
                    'graph': json.dumps(EMPTY_GRAPH, ensure_ascii=False),
                    'create_by': operator,
                    'create_time': datetime.now(),
                },
            )
        ...
        try:
            ...
            db.add(
                Task(
                    ...
                    status=1,
                    trigger_type=1,
                    run_queue=src.run_queue or 'default',
    ...
    async def get_detail(cls, db: AsyncSession, dag_task_id: str) -> dict:
        ...
        return {
            ...
            'crontab': t.crontab,
            'runQueue': t.run_queue,
            'runType': t.run_type or 1,
            'retry': t.retry,
    ...
    async def update_settings(cls, db: AsyncSession, dag_task_id: str, req: Any, operator: str) -> CrudResponseModel:
        ...
        try:
            ...
            if req.trigger_type == 2:
                job = JobModel(
                    jobName=f'DAG_{dag_task_id}',
                    jobGroup='default',
                    jobExecutor='default',
                    invokeTarget='module_task_schedule.dispatch.run_dag',
                    jobArgs=str(dag_task_id),
                    cronExpression=req.crontab,
                    misfirePolicy='2',
                    concurrent='1',
                    status='0' if req.status == 1 else '1',
                    createBy=operator,
                    remark=req.remark,
                )
