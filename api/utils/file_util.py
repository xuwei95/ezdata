...
class FileUtil:
    ...
    def stage_file_deletions(cls, file_infos: list[FileStorageInfo]) -> list[StagedFile]:
        ...
        try:
            for file_info in file_infos:
                # 对象存储文件不落本地回收区：软删仅改库状态，对象留桶待清理
                if getattr(file_info, 'storage_type', 'local') != 'local':
                    continue
    ...
    def prepare_deleted_files_for_purge(cls, file_infos: list[FileStorageInfo]) -> list[StagedFile]:
        ...
        staged_files = []
        for file_info in file_infos:
            # 对象存储文件的清理走 purge_object_storage_files，不在本地回收区校验
            if getattr(file_info, 'storage_type', 'local') != 'local':
                continue
            staged_file = cls._get_staged_file_paths(file_info)
            if staged_file.source_path.exists():
    ...
    @staticmethod
    def build_object_storage_key(access_type: str, storage_key: str) -> str:
        """对象存储物理键：单桶内以 access_type 作前缀区分公私（与上传时一致）。"""
        return f'{access_type}/{storage_key}'

    @classmethod
    def purge_object_storage_files(cls, file_infos: list[FileStorageInfo]) -> None:
        """永久删除对象存储（s3）中的文件对象；本地文件由 purge_deleted_files 处理。

        对象存储回收站采用「DB 状态软删 + 对象保留至清理」模型：软删/恢复仅改库、对象留桶，
        清理时才真正删对象。此处按对象键删除，不存在视为已删（幂等）。
        """
        for file_info in file_infos:
            if getattr(file_info, 'storage_type', 'local') != 's3':
                continue
            object_key = cls.build_object_storage_key(file_info.access_type, file_info.storage_key)
            try:
