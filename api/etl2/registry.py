...
from etl2.data_models.mindsdb_sql import MindsDBSqlModel
from etl2.data_models.mindsdb_table import MindsDBTableModel
...
CUSTOM_HANDLERS = {
    # AkShare - 金融数据接口（只读）
    'akshare:None': 'etl2.data_models.akshare_api.AkShareModel',
    'akshare:akshare_api': 'etl2.data_models.akshare_api.AkShareModel',

    # CCXT - 加密货币交易所接口（只读）
    'ccxt:None': 'etl2.data_models.ccxt_api.CCxtModel',
    'ccxt:ccxt_api': 'etl2.data_models.ccxt_api.CCxtModel',

    # Kafka - 实时流数据（可读可写）
    'kafka:None': 'etl2.data_models.kafka_topic.KafkaTopicModel',
    'kafka:kafka_topic': 'etl2.data_models.kafka_topic.KafkaTopicModel',

    # MySQL Binlog - CDC 变更数据捕获（只读）
    'mysql:mysql_binlog': 'etl2.data_models.mysql_binlog.MysqlBinlogModel',
    ...
    def get_reader_map(self):
        ...
        for handler_name in self.mindsdb_data_handlers:
            ...
            if key_sql not in reader_map:
                reader_map[key_sql] = 'etl2.data_models.mindsdb_sql.MindsDBSqlModel'

            # 添加默认类型
            key_none = f"{source_type}:None"
            if key_none not in reader_map:
                reader_map[key_none] = 'etl2.data_models.mindsdb_sql.MindsDBSqlModel'

            # 添加 table 类型
            key_table = f"{source_type}:{source_type}_table"
            if key_table not in reader_map:
                reader_map[key_table] = 'etl2.data_models.mindsdb_table.MindsDBTableModel'
    ...
    def get_writer_map(self):
        ...
        for handler_name in self.mindsdb_data_handlers:
            ...
            if key_table not in writer_map:
                writer_map[key_table] = 'etl2.data_models.mindsdb_table.MindsDBTableModel'
