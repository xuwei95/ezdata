-- 数据集成 / 流式摄取 演示任务(依赖 demo_mysql / demo_minio / demo_binlog / demo_es 数据源)
-- 重复执行安全:先删后插
DELETE FROM task WHERE id IN ('demo_etl_m2m','demo_etl_m2file','demo_etl_tf','demo_stream_cdc','demo_etl_m2es','demo_etl_es2m');
...
('demo_etl_m2es','DataIntegrationTask',1,'演示-MySQL→Elasticsearch',
 '{"extract":{"datasource_code":"demo_mysql","object":"demo_orders","native":"SELECT id,order_no,product_id,qty,amount,status,customer,order_time FROM demo_orders LIMIT 1000"},"transform":{"enabled":false,"code":""},"load":{"datasource_code":"demo_es","table":"es_orders","mode":"replace","dataset":"public","format":"csv"}}',
 1,1,'default',1,0,0,'admin',now(),100,'抽 demo_orders 写入 ES 索引 es_orders'),

('demo_etl_es2m','DataIntegrationTask',1,'演示-Elasticsearch→MySQL',
 '{"extract":{"datasource_code":"demo_es","object":"es_orders","native":{"index":"es_orders","body":{"query":{"match_all":{}},"size":1000}}},"transform":{"enabled":false,"code":""},"load":{"datasource_code":"demo_mysql","table":"es_orders_back","mode":"replace","dataset":"dwh","format":"csv"}}',
 1,1,'default',1,0,0,'admin',now(),100,'从 ES 读文档(DSL)装载回 MySQL');
