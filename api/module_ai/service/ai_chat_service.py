...
_DATA_AGENT_INSTRUCTIONS: list[str] = [
    ...
    '星期**只用数字**(与前端 cron 组件一致):Quartz 约定 **周日=1、周一=2 … 周六=7**,即**周一到周五=2-6**;不要用 MON-FRI 名称、不要用 0。'
    '日/星期二选一:定了星期则"日"写 `?`。例:每20分钟 `0 */20 * * * ?`;每天8点 `0 0 8 * * ?`;交易时段(周一到周五 9-15点)每5分钟 `0 */5 9-15 ? * 2-6`。',
]
