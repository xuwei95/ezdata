...
class TaskAgentTools(Toolkit):
    ...
    def __init__(self, ui_actions: list | None = None, **kwargs: Any) -> None:
        ...
        super().__init__(
            name='task_propose',
            tools=[
                self.propose_data_integration_task,
                self.propose_code_extract_task,
                self.propose_python_task,
                self.propose_shell_task,
                self.find_tasks,
                self.get_task_detail,
                self.get_task_run_history,
                self.propose_task_update,
                self.propose_task_copy,
            ],
            **kwargs,
        )
    ...
    def _push(self, template_code: str, name: str, params: dict, schedule_cron: str, summary: str) -> str:
        ...
        return (
            f'已向用户弹出「{name}」的任务确认表单({template_code},{sched}),'
    ...
    def propose_data_integration_task(
        ...
        **务必先用 get_table_schema 摸清源的表/函数/字段及其原生查询写法**(尤其:akshare/ccxt 的查询是
        ...
        :param schedule_cron: 可选,7 段 Quartz cron(北京时区);写法规则见 load_skill('task_scheduling');留空=单次
    ...
    def propose_code_extract_task(
        ...
        return self._push(
            'DataIntegrationTask', name, params, schedule_cron, f'代码取数 → {target_datasource_code}.{target_table}'
        )
    ...
    def get_task_detail(self, task_id: str) -> str:
        ...
        lines = [
            f'任务「{task["name"]}」 task_id={task["id"]}  模板={_TPL_LABEL.get(tpl, tpl)}  触发={sched}  状态={st}'
        ]
        # 运行配置(与任务实际字段对齐:重试/超时/队列)
        _to = task.get('timeout')
        timeout_txt = '全局默认' if not _to else ('不限' if _to < 0 else f'{_to}s')
        lines.append(
            f'运行配置: 重试={task.get("retry") or 0} 次(间隔 {task.get("countdown") or 0}s)  '
        ...
        if tpl in ('PythonTask', 'ShellTask'):
            ...
            lines.append(
                f'装载: 目标={ld.get("datasource_code")}  表/索引={ld.get("table")}  '
    ...
    def propose_task_update(
        ...
        **也能就地改内容**:「把 XX 任务的转换逻辑改成 …」「把取数语句换成 …」「改一下这个 Python 任务的代码」。
        ...
        :param schedule_cron: 新定时,7 段 Quartz cron(北京时区);写法规则见 load_skill('task_scheduling');留空=不改
        ...
        if tpl == 'DataIntegrationTask':
            ...
            if source_query and source_query.strip():
                ex['native'] = source_query.strip()
                changes.append('更新取数语句')
            if extract_code and extract_code.strip():
                ex['mode'] = 'code'
                ex['code'] = extract_code
                changes.append('更新取数代码')
        ...
        return f'已向用户弹出「{new_name}」的任务修改确认表单({chg}),等待用户在表单上确认保存。请不要再自行改动该任务。'
