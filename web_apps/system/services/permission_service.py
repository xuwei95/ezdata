...
class PerMissionService(object):
    ...
    def get_obj_list(self, req_dict):
        ...
        query = get_base_query(PerMission)
        show_children = True
        if parent_id != '':
            query = get_base_query(PerMission).filter(or_(PerMission.id == parent_id, PerMission.parent_id == parent_id))
        if search_text != '':
    ...
    def get_all_menus(self, req_dict):
        ...
        :return:
        '''
        obj_list = get_base_query(PerMission).filter(PerMission.menu_type.in_([0, 1])).all()
        result = []
        parent_objs = [obj for obj in obj_list if obj.parent_id is None]
        for obj in parent_objs:
    ...
    def get_user_menus(self, user_info):
        ...
        obj_list = get_base_query(PerMission).filter(PerMission.id.in_(prem_id_list)).filter(
    ...
    def get_all_auth(self, req_dict):
        ...
        :return:
        '''
        obj_list = get_base_query(PerMission).filter(PerMission.menu_type == 2).all()
        result = []
        for obj in obj_list:
    ...
    def get_user_auth(self, user_info):
        ...
        '''
        prem_id_list = self.get_user_prem_ids(user_info)
        prem_objs = get_base_query(PerMission).filter(PerMission.id.in_(prem_id_list)).filter(
    ...
    def get_role_tree_list(self, req_dict):
        ...
        :return:
        '''
        obj_list = get_base_query(PerMission).all()
        treeList = []
        parent_objs = [obj for obj in obj_list if obj.parent_id is None]
        for obj in parent_objs:
    ...
    def update_obj(self, req_dict):
        ...
        for k in req_dict:
            setattr(obj, k, req_dict[k])
        child_objs = get_base_query(PerMission).filter(PerMission.parent_id == obj.id).all()
